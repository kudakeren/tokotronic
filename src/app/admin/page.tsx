    ...
    <DashboardShell title="Admin Dashboard" admin>
      <div className="grid gap-6">
        <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
          <Card className="overflow-hidden border border-red-100 bg-white text-slate-950 shadow-[0_24px_80px_rgba(16,185,129,.12)]">
            <CardContent className="relative p-6">
              <div className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-red-400 via-red-300 to-zinc-300" />
              <div className="relative">
                <TrendingUp className="mb-5 h-8 w-8 text-red-500" />
          ...
          <Card className="overflow-hidden border bg-white shadow-sm dark:bg-slate-950">
            <CardContent className="relative p-5">
              <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-zinc-300 via-red-400 to-zinc-300" />
              <div className="space-y-4">
                <AdminQueue icon={<WalletCards className="h-5 w-5" />} title="Deposit pending" text={`${pendingDeposits} deposit menunggu approval`} tone="text-zinc-600 bg-zinc-500/10" />
                <AdminQueue icon={<AlertTriangle className="h-5 w-5" />} title="Transaksi gagal" text={`${failed} transaksi perlu dicek/refund`} tone="text-slate-600 bg-slate-500/10" />
                <AdminQueue icon={<ShieldCheck className="h-5 w-5" />} title="Audit stok & saldo" text="Mutasi saldo dan listing gadget tercatat real-time" tone="text-red-600 bg-red-500/10" />
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <StatCard title="Deposit pending" value={pendingDeposits} tone="slate" />
          <StatCard title="Sukses" value={success} tone="emerald" />
          <StatCard title="Gagal" value={failed} tone="slate" />
        </div>

        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            ["/admin/users", "Manage Users", "Role, status, saldo, dan detail user", Users, "bg-red-500/10 text-red-600"],
            ["/admin/products", "Manage Gadget", "Harga jual, kondisi, seller, kategori", Boxes, "bg-red-500/10 text-red-600"],
            ["/admin/deposits", "Approve Deposits", "Validasi deposit dan tambah saldo", Landmark, "bg-zinc-500/10 text-zinc-600"],
            ["/admin/transactions", "Refund Center", "Update status dan refund gagal", CreditCard, "bg-red-500/10 text-red-600"]
          ].map(([href, title, text, Icon, tone]) => (
            <Link key={String(title)} href={String(href)} className="group rounded-3xl border bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-red-400 hover:shadow-xl dark:bg-slate-950">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
                  ...
                  {["Tronic Second Store", "Apple Preloved Hub", "Android Reseller ID", "Audio & Gaming Corner"].map((name, index) => (
                    <div key={name} className="rounded-2xl border bg-background p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={index === 1 ? "rounded-full bg-zinc-500/10 px-2 py-1 text-xs font-bold text-zinc-600" : "rounded-full bg-red-500/10 px-2 py-1 text-xs font-bold text-red-600"}>{index === 1 ? "Maintenance" : "Active"}</span>
                      </div>
                      <div className="mt-3 h-2 rounded-full bg-muted"><div className="h-2 rounded-full bg-red-500" style={{ width: `${index === 1 ? 68 : 92 - index * 8}%` }} /></div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <Card className="border bg-white shadow-sm dark:bg-slate-950">
            <CardContent className="p-6">
              <DatabaseZap className="mb-5 h-8 w-8 text-red-500" />
              ...
              <div className="mt-4 space-y-3 text-sm">
                {["Database gadget terpisah", "Seed produk gadget tersedia", "Role admin aktif", "Order dan mutasi saldo tercatat"].map((item) => (
                  <div key={item} className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4 text-red-500" />{item}</div>
                ))}
              </div>
            </CardContent>
          </Card>
...
function AdminHeroMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl bg-red-50 p-4"><p className="text-xs font-bold text-red-700">{label}</p><p className="mt-1 text-xl font-black text-red-950">{value}</p></div>;
}
